...
FrontControllerServletV5의 service 메서드 안 // MyHandlerAdapter adapter = getHandlerAdapter(handler); // <br/>
...
viewName 과 함께 // view.render(mv.getModel(), request, response); // 에서 최종적으로 MyView로 값을 넘겨주고 > <br/>
...
먼저 getHandler메서드가 요청받은 url을 통해 handlerMappingMap에서 알맞은 컨트롤러를 호출하고 리턴한다
...
여기서 넘겨받은 MemberFormControllerV3을 통해 알맞은 어댑터(여기서는 ControllerV3HandlerAdapter의)를 찾을 수 있다
MemberFormControllerV3은 ControllerV3의 구현체이기에 ControllerV3이 호출되고 
MyHandlerAdapter는 뭐가 들어올지 모르는 상태로 ControllerV3만 가지고 클래스 내의 supports를 돌려 ControllerV3을 통과하는 어탭터를 호출한다
